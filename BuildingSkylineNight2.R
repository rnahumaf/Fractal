...
plot(x = vx, y = vy, type="s", ylim=c(min.atual, (3*max(vy))), xlim=c(30,390), axes = F, xlab = "", ylab = "")

#Tela preta de fundo
polygon(x = c(0,0,450,450), y = c(min.atual, (3*max(vy)), (3*max(vy)), min.atual), col=c(rgb(0,0,0)))

# Estrelas
points(x = sample(1:450, 50, replace = T), y = sample(min.atual:(3*max(vy)), 50, replace = T), col="white", pch=8, cex = exp(seq(0.05,(0.05*50), by = 0.05))/10)
...
points(x = janx, y = jany, col=sample(c(rgb(0,0,0), 
                                        rgb(0.1,0.1,0.1), 
                                        rgb(0.2,0.2,0.2), 
                                        rgb(0.4,0.4,0.4), 
                                        rgb(0.5,0.5,0.5), 
                                        ...
                                        rgb(0.8,0.8,0.8), 
                                        rgb(0.9,0.9,0.9), 
                                        rgb(1,1,1)),50, replace = T), pch=15, cex = fator/10)
